...
function split_and_return(ws, fr)
    -- split_at :: WTiling -> WFrame -> String -> Bool -> WFrame
    f = WTiling.split_at(ws, fr, 'right', true)
    -- switch_to :: WMPlex (Frame or Screen) -> WRegion (anything) -> ()
    ioncore.goto_focus(f)
    return f
end
...
defbindings("WTiling", {
    ...
    kpress(META.."I", "WTiling.split_at(_, _sub, 'left', false)"),
    -- kpress(META.."I", "split_and_return(_, _sub)"),

    bdoc("Split current into rows.", "vsplit"),
    kpress(ALTMETA.."I", "WTiling.split_at(_, _sub, 'top', false)"),
